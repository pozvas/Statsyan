...
def get_start_info(parser: DemoParser) -> pd.DataFrame:
    ...
    team = team.rename(columns={"user_steamid": "steamid"})
    wanted_props = [
        "team_name",
        "crosshair_code",
        "rank",
    ...
    team_name = team_name.merge(
        team[["steamid", "oldteam"]], how="left", on=["steamid"]
    )
    team_name["global_team_name"] = team_name.apply(
        lambda row: "Team " + str(row["oldteam"] - 1), axis=1
    )
    team_name["is_tie"] = (
        team_name["team_rounds_total"].drop_duplicates().size == 1
    )
    win_team = team_name.loc[team_name["team_rounds_total"].idxmax()][
        "global_team_name"
    ]
    team_name["is_win"] = team_name.apply(
        lambda row: (
            True
            if not row["is_tie"] and row["global_team_name"] == win_team
            else False
        ),
        axis=1,
    )
...
def get_buy_type_by_rounds(parser: DemoParser) -> pd.DataFrame:
    ...
    def get_buy_type_name(row):
        second_pistol = 12 if row["comp_rank_type"] != 7 else 8

        if row["total_rounds_played"] in [0, second_pistol]:
            pistol_record = None
        ...
        for buy_type in BUYTYPE:
            if (
                buy_type["equipment_value_from"]
                <= row["equip_value"]
                <= buy_type["equipment_value_to"]
                and buy_type["is_pistol"] == 0
            ):
    ...
    df = df.merge(
        start_info[["steamid", "global_team_name"]], how="left", on="steamid"
    )
    ...
    group_df = group_df.merge(
        group_df, on=["total_rounds_played"], suffixes=("", "_enemy")
    )
    group_df = group_df[
        group_df["team_name"] != group_df["team_name_enemy"]
...
def get_rounds_count(parser: DemoParser) -> pd.DataFrame:
    ...
    round_start = parser.parse_ticks(
        ["player_steamid", "team_name", "total_rounds_played"],
        ticks=round_start_ticks["tick"],
    )
    round_start = round_start.merge(
...
def get_adr(parser: DemoParser) -> pd.DataFrame:
    ...
    ticks = parser.parse_ticks(
        ["team_name", "health", "total_rounds_played"],
        ticks=pd.concat([df["tick"], df["tick"] - 1]),
    )

    for idx, row in df.iterrows():
        # print(row)
        df.loc[idx, "attacker_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["attacker_steamid"]))
        ]["team_name"].iat[0]
        df.loc[idx, "user_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
    ...
    filtred_df["damage"] = filtred_df.apply(
        lambda row: (
            row["was_health"] if row["health"] == 0 else row["dmg_health"]
        ),
        axis=1,
    )

    filtred_df = filtred_df.merge(
        get_buy_type_by_rounds(parser),
...
def calculate_trades(
    kills: pd.DataFrame, trade_ticks: int = 128 * 5
) -> pd.DataFrame:
    ...
    for r in rounds:
        ...
        for _, row in kills_round.iterrows():
            ...
            if (
                row["victim_steamid"]
                in kills_in_trade_window["attacker_steamid"].to_numpy()
            ):
                last_kill_by_attacker = None
                for __, attacker_row in kills_in_trade_window.iterrows():
                    if (
                        attacker_row["attacker_steamid"]
                        == row["victim_steamid"]
                    ):
...
def get_kast_rounds(parser: DemoParser) -> pd.DataFrame:
    deaths = parser.parse_event(
        "player_death", other=["total_rounds_played"]
    ).rename(columns={"user_steamid": "victim_steamid"})
    ticks = parser.parse_ticks(
        ["team_name", "total_rounds_played"], ticks=deaths["tick"].to_list()
    )

    def get_team_name(row, ticks, steamid_column):
        return ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row[steamid_column]))
        ]["team_name"].iat[0]

    deaths["attacker_team_name"] = deaths[
        deaths["attacker_steamid"].notna()
    ].apply(lambda row: get_team_name(row, ticks, "attacker_steamid"), axis=1)
    deaths["assister_team_name"] = deaths[
        deaths["assister_steamid"].notna()
    ].apply(lambda row: get_team_name(row, ticks, "assister_steamid"), axis=1)
    deaths["victim_team_name"] = deaths.apply(
        lambda row: get_team_name(row, ticks, "victim_steamid"), axis=1
    )
    ...
    kills = (
        ...
        .loc[
            :,
            ["attacker_team_name", "attacker_steamid", "total_rounds_played"],
        ]
        .drop_duplicates()
        .rename(
            columns={
    ...
    assists = (
        ...
        .loc[
            :,
            ["assister_team_name", "assister_steamid", "total_rounds_played"],
        ]
        .drop_duplicates()
        .rename(
            columns={
    ...
    survive = (
        ...
        .rename(columns={"user_stemaid": "steamid"})
        .reset_index(drop=True)
    )

    survive = (
        survive[survive["health"] > 0]
        .groupby(["steamid", "team_name", "total_rounds_played"])
        .tail(1)
    )
    survive["total_rounds_played"] = survive.apply(
        lambda row: row["total_rounds_played"] - 1, axis=1
    )
    survive = survive[
        [
            "team_name",
    ...
    survive = survive.merge(
        buy_type_by_rounds, on=["total_rounds_played", "team_name"]
    )
...
def get_kd(parser: DemoParser) -> pd.DataFrame:
    ...
    filter_deaths["attacker_team_name"] = filter_deaths.apply(
        lambda row: get_team_name(row, ticks, "attacker_steamid"), axis=1
    )
    filter_deaths["assister_team_name"] = filter_deaths[
        filter_deaths["assister_steamid"].notna()
    ].apply(lambda row: get_team_name(row, ticks, "assister_steamid"), axis=1)
    filter_deaths["user_team_name"] = filter_deaths.apply(
        lambda row: get_team_name(row, ticks, "user_steamid"), axis=1
    )

    columns = {"kills": "attacker", "assists": "assister", "deaths": "user"}
    result = None

    buy_type_by_rounds = get_buy_type_by_rounds(parser)

    for column_name, prefix in columns.items():
        df = filter_deaths.rename(
            columns={
...
def get_multykills(parser: DemoParser) -> pd.DataFrame:
    deaths = parser.parse_event("player_death", other=["total_rounds_played"])
    ticks = parser.parse_ticks(["team_name"], ticks=deaths["tick"].to_list())
    deaths = deaths[deaths["attacker_steamid"].notna()].rename(
        columns={
            "attacker_team_name": "team_name",
            "attacker_steamid": "steamid",
        }
    ...
    for idx, row in deaths.iterrows():
        deaths.loc[idx, "team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["steamid"]))
        ]["team_name"].iat[0]
    ...
    multykills = (
        ...
        .size()
        .reset_index(name="kills_in_round")
    )
    result = None

    for i in range(1, 6):
        kills = (
            multykills[multykills["kills_in_round"] == i]
            .groupby(
                [
                    "team_name",
                    "steamid",
    ...
    for round_idx in range(0, max_round):
        for _, death in deaths.iterrows():
            if death["total_rounds_played"] == round_idx:
                ...
                ct_alive = subdf[
                    (subdf["team_name"] == "CT") & (subdf["is_alive"] == True)
                ]
                t_alive = subdf[
                    (subdf["team_name"] == "TERRORIST")
                    & (subdf["is_alive"] == True)
                ]
                ...
                if (
                    len(ct_alive) == 1
                    and len(t_alive) != 0
                    and round_ends.loc[round_idx, "winner"]
                    == ("CT" if is_win else "T")
                ):
                    result.loc[len(result)] = pd.Series(
                        {
                            "steamid": ct_alive["steamid"].iat[0].astype(str),
                ...
                if (
                    ...
                    result.loc[len(result)] = pd.Series(
                        {
                            "steamid": t_alive["steamid"].iat[0].astype(str),
                            "total_rounds_played": round_idx,
...
def get_clutches(parser: DemoParser) -> pd.DataFrame:
    buy_type_by_rounds = get_buy_type_by_rounds(parser)
    full_win_clutches = get_full_clutches_info(parser).merge(
        buy_type_by_rounds, on=["total_rounds_played", "team_name"]
    )
    full_lose_clutches = get_full_clutches_info(parser, is_win=False).merge(
        buy_type_by_rounds, on=["total_rounds_played", "team_name"]
    )
    result = None
    ...
    for i in range(1, 6):
        win_clutches = (
            full_win_clutches[full_win_clutches["against"] == i]
            .groupby(
                [
                    "steamid",
                    "team_name",
            ...
            .size()
            .reset_index(name=f"win_clutches_1x{i}")
        )
        lose_clutches = (
            full_lose_clutches[full_lose_clutches["against"] == i]
            .groupby(
                [
                    "steamid",
                    "team_name",
            ...
            .size()
            .reset_index(name=f"loss_clutches_1x{i}")
        )

        tmp = lose_clutches.merge(
            win_clutches,
            how="outer",
            on=[
                "steamid",
...
def get_first_kills(parser: DemoParser) -> pd.DataFrame:
    ...
    for _, kill in firt_kills.iterrows():
        attacker_team = df[
            (df["tick"] == kill["tick"])
            & (df["steamid"] == int(kill["attacker_steamid"]))
        ]["team_name"].iat[0]
        victim_team = df[
            (df["tick"] == kill["tick"])
            & (df["steamid"] == int(kill["user_steamid"]))
        ]["team_name"].iat[0]
        result.loc[len(result)] = pd.Series(
            {
...
def get_utility_damage(parser: DemoParser) -> pd.DataFrame:
    ...
    ticks = parser.parse_ticks(
        ["team_name", "health"], ticks=pd.concat([df["tick"], df["tick"] - 1])
    )

    for idx, row in util_dmg.iterrows():
        util_dmg.loc[idx, "attacker_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["attacker_steamid"]))
        ]["team_name"].iat[0]
        util_dmg.loc[idx, "user_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["user_steamid"]))
        ]["team_name"].iat[0]
    ...
    util_dmg_sides = (
        ...
        .sum()
        .reset_index(name="utility_damage")
    )
...
def get_enemy_flashed(parser: DemoParser) -> pd.DataFrame:
    ...
    flashes = parser.parse_ticks(
        ["enemies_flashed_total", "team_name", "total_rounds_played"],
        ticks=rounds_end,
    )
    ...
    enemy_flashed_sides = (
        ...
        .max()
        .reset_index(name="enemy_flashed")
    )

    return enemy_flashed_sides.fillna(0)[
        enemy_flashed_sides.fillna(0)["enemy_flashed"] != 0
    ]
...
def get_flash_assists(parser: DemoParser) -> pd.DataFrame:
    ...
    for idx, row in flash_assists.iterrows():
        flash_assists.loc[idx, "assister_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["assister_steamid"]))
        ]["team_name"].iat[0]

    flash_assists = flash_assists.rename(
        columns={
            "assister_team_name": "team_name",
            "assister_steamid": "steamid",
        }
    ...
    flash_assists_sides = (
        ...
        .size()
        .reset_index(name="flash_assists")
    )
...
def get_kills_and_damage_by_weapon(parser: DemoParser) -> pd.DataFrame:
    ...
    return (
        hits_by_weapon_and_hitgroup.merge(
            kills_by_weapon_and_hitgroup,
            how="left",
...
def get_weapon_fired(parser: DemoParser) -> pd.DataFrame:
    ...
    for idx, row in fires.iterrows():
        fires.loc[idx, "user_team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["user_steamid"]))
        ]["team_name"].iat[0]

    fires = fires.rename(
        columns={"user_steamid": "steamid", "user_team_name": "team_name"}
    )

    return (
        fires.groupby(["steamid", "team_name", "weapon"])
        .size()
...
def get_weapon_stat(parser: DemoParser) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    deaths = parser.parse_event(
        "player_death", other=["total_rounds_played"]
    ).rename(columns={"attacker_steamid": "steamid"})[
    ...
    fires = parser.parse_event(
        "weapon_fire", other=["total_rounds_played"]
    ).rename(columns={"user_steamid": "steamid"})[
        ["tick", "steamid", "weapon"]
    ]
    fires = fires[fires["steamid"].notna()]

    hurt = parser.parse_event(
        "player_hurt", other=["total_rounds_played"]
    ).rename(columns={"attacker_steamid": "steamid"})[
        [
            "tick",
            "steamid",
    ...
    def set_right_name(row):
        ...
        return weapon_name.iat[0]

    hurt["weapon"] = hurt.apply(lambda row: set_right_name(row), axis=1)
    ...
    ticks = parser.parse_ticks(
        ["team_name", "health"],
        ticks=pd.concat([all_fires["tick"], all_fires["tick"] - 1]),
    )

    for idx, row in all_fires.iterrows():
        all_fires.loc[idx, "team_name"] = ticks[
            (ticks["tick"] == row["tick"])
            & (ticks["steamid"] == int(row["steamid"]))
        ]["team_name"].iat[0]
        if row["is_damage"] != 0:
            all_fires.loc[idx, "user_team_name"] = ticks[
                (ticks["tick"] == row["tick"])
                & (ticks["steamid"] == int(row["user_steamid"]))
            ]["team_name"].iat[0]
    ...
    all_fires = (
        ...
        .agg(
            ...
            hitgroup_hurt=(
                "hitgroup_hurt",
                "min",
            ),  # что-то придумать и поправить не макс
        )
        .reset_index()
    )
    ...
    all_fires["damage"] = all_fires.apply(
        lambda row: (
            row["was_health"]
            if row["dmg_health"] > row["was_health"]
            else row["dmg_health"]
        ),
        axis=1,
    )
    all_fires["hitgroup"] = all_fires.apply(
        lambda row: (
            row["hitgroup"] if row["hitgroup"] != 0 else row["hitgroup_hurt"]
        ),
        axis=1,
    )
    all_fires["weapon"] = all_fires.apply(
        lambda row: (
            "weapon_knife" if "knife" in row["weapon"] else row["weapon"]
        ),
        axis=1,
    )
    ...
    fires_all = (
        group_df.groupby(["steamid", "weapon", "team_name"])["fires_count"]
        .sum()
...
def get_duels(parser: DemoParser) -> pd.DataFrame:
    ...
    duels = (
        deaths.groupby(["attacker_steamid", "user_steamid"])
        .size()
        .reset_index(name="duels")
        .rename(columns={"user_steamid": "victim_steamid"})
    )
    first_kills_duels = (
        first_kills.groupby(["attacker_steamid", "victim_steamid"])
        .size()
...
def get_rounds(parser: DemoParser) -> pd.DataFrame:
    ...
    end = parser.parse_ticks(
        ["round_win_reason", "total_rounds_played"], ticks=ticks_end["tick"]
    )[["round_win_reason", "total_rounds_played"]].drop_duplicates()
...
def get_stats(parser: DemoParser):
    ...
    result = result.merge(
        get_start_info(parser)[["steamid", "global_team_name"]],
        how="left",
        on=["steamid"],
    )
