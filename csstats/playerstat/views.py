...
from playerstat.mixins import PlayerListMixin, PlayerAuthCodeMixin
...
class PlayerStatsView(SteamUserBaseMixin, PlayerAuthCodeMixin, DetailView):
    model = Player
    ...
    def get_object(self, queryset=None) -> QuerySet[Any]:
        ...
        self.player_stat = (
            ...
            .annotate(
                ...
                win_count=Count('demo__id', distinct=True, filter=Q(team=F('demo__win_team'))),
                rating=(
                    (
                        0.73 * Sum('scoreboard__kast_rounds') +
                        0.3591 * Sum('scoreboard__kills') -
...
class AuthCodeUpdateView(SteamUserBaseMixin, LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            'playerstat:stats',
            kwargs={'player_id': self.kwargs['player_id']}
        )
